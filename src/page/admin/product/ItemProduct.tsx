...
const ItemProduct = ({ product, stt, handleSelectItem, selectedIds, setLoad }: ItemProductProps) => {
  ...
  const handleRestore = async (id: number) => {
    setIsLoading(true);
    try {
      await axios.patch(`${SERVER_HOST}/products/change/all?status=INACTIVE`, { ids: [id] });
  ...
  return (
    <tr className="hover:bg-gray-100 dark:hover:bg-gray-700">
      ...
      <td className="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
        <img src={`${SERVER_HOST}/${image[0]}`} alt="Uploaded" className="w-20 h-20 object-cover rounded" />
      </td>

      <td className="text-base font-semibold text-gray-900 dark:text-white">{formatCurrency(product.price)}</td>
      <td className="text-base font-semibold text-gray-900 dark:text-white">{product.isDiscount ? "Có" : "Không"}</td>
      ...
      {product.status == "DELETED" ? (
        ...
        <td className="p-4 space-x-2 whitespace-nowrap">
          <Link
            to={`/admin/crud/edit/product/${product.id}`}

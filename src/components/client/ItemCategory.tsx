...
const ItemCategory = ({ item }: { item: ICategory }) => {
  return (
    <Link to={""} key={item.id} className="flex flex-col items-center p-3 group">
      <img className="h-[80px] w-[80px] object-cover rounded-full transition-transform duration-500 group-hover:rotate-45" src={`${SERVER_HOST}/${item.image}`} alt={item.name} />
